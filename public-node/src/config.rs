...
impl Config {
    pub fn load(keypair_path_arg: Option<&str>) -> Result<Self> {
        ...
        let keypair_path = if let Some(path) = keypair_path_arg {
            PathBuf::from(path)
        } else {
            let env_path = std::env::var("PUBLIC_NODE_KEYPAIR_PATH")
                .context("PUBLIC_NODE_KEYPAIR_PATH environment variable not set. Provide keypair path as command-line argument or set PUBLIC_NODE_KEYPAIR_PATH")?;
