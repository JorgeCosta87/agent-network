...
use services::{NodeRegistryService, DacManagerService};
...
async fn main() -> Result<()> {
    ...
    let dac_manager_service = Arc::new(DacManagerService::new(Arc::clone(&solana_adapter)));
    let node_registry_service = NodeRegistryService::new(Arc::clone(&dac_manager_service));
    node_registry_service.check_and_claim_pending_node(&config.node_pubkey()).await?;
    
    let mut agent_service = AgentService::new(Arc::clone(&dac_manager_service));
