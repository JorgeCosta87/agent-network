...
impl AgentService {
    pub fn new(dac_manager: Arc<DacManagerService>) -> Self {
        Self { dac_manager, agents_registry: Arc::new(Mutex::new(HashMap::new()))}
    }
    ...
    pub async fn monitor_and_register_agents(
        ...
        if let Some(agents) = self.dac_manager.get_agents_accounts(node_pubkey).await? {
            for agent in agents {
                if agent.1.status == AgentStatus::Pending {
                    println!("Activating pending agent: {:?}", agent.0);
                    if let Err(e) = self.dac_manager.activate_agent(&agent.1).await {
    ...
    async fn add_agent(
        ...
        let agent_address = self.dac_manager.derive_agent_pda(&agent.owner, agent.agent_id)?;
        self.agents_registry.lock().await.insert(agent_address, agent);
    ...
    pub fn start_watching_agent_accounts(
        ...
        tokio::spawn(async move {
            ...
            let watch_handle = dac_manager.watch_agent_accounts(&node_pubkey, tx);
                    ...
                    result = rx.recv() => {
                        match result {
                            Some(agent) => {
                                ...
                                let agent_address = dac_manager.derive_agent_pda(&agent.owner, agent.agent_id).unwrap(); //TODO: pass the agent and the address
                                if agent.status == AgentStatus::Pending {
                                    println!("Activating pending agent: {:?}", agent_address);
                                    if let Err(e) = dac_manager.activate_agent(&agent).await {
